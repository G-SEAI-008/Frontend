  ...
  return (
    <>
      <div className='flex w-36 justify-between border-2'>
        <button onClick={handleClick} className='w-12 cursor-pointer bg-green-400 p-4 font-bold'>
          +
        </button>
        <span className='p-4'>{counter}</span>
        <button onClick={handleClick} className='w-12 cursor-pointer bg-red-400 p-4 font-bold'>
          -
        </button>
      </div>

      <button onClick={() => setCounter(0)} className='mt-1 cursor-pointer bg-gray-300 p-2'>
        Clear
      </button>
    </>
  );
