...
test('vergleicht Objektinhalte mit toStrictEqual', () => {
  const user = { name: 'Alice', age: 28 };
  expect(user).toStrictEqual({ name: 'Alice', age: 28 });

  // * Besteht: toEqual ignoriert hier die zusätzliche Eigenschaft
  // oxlint-disable-next-line vitest/prefer-strict-equal
  // expect({ name: 'Alice', age: 28 }).toEqual({ name: 'Alice' });
  // * Schlägt fehl: toStrictEqual ignoriert nicht zusätzliche Eigenscahften
...
// * toBeTruthy(): Würde der Wert in einer if-Bedingung als wahr/truthy gelten?
