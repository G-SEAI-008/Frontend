 ...
 *
 * @param {boolean} includeBreeds - Whether to include breed-specific facts
 * @returns {Promise<string>} Promise that resolves to a cat fact string
 */
export async function fetchRandomCatFact(includeBreeds = true): Promise<string> {
  const generalFacts = [
